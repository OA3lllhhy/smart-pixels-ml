...
class OptimizedDataGenerator(tf.keras.utils.Sequence):
    ...
    def process_file_parallel(self):
        # Add debugging to check if files exist
        if not self.recon_files:
            raise ValueError(f"No files found matching pattern in data_directory_path. "
                            f"Check if the directory and file pattern are correct.")
    
        print(f"Found {len(self.recon_files)} files to process")
        print(f"First file: {self.recon_files[0] if self.recon_files else 'None'}")
